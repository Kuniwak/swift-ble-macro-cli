...
struct StartREPL: AsyncParsableCommand {
    ...
    mutating func run() async throws {
        ...
        let logURL = URLExtensions.appending(path: "ble-repl.log", toURL: fileManager.temporaryDirectory, isDirectory: false)
        print(toStderr: "log file: \(logURL.path())")
        
        do {
            if !fileManager.fileExists(atPath: URLExtensions.path(ofURL: logURL)) {
